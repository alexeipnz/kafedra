...
?>
    ...
    <ul class="nav nav-pills">
        <?php if ($table === 'publications') { ?>
            <li role="presentation" class="active">
                <a href="javascript:reloadWithParam('table','publications')">Публикации</a>
            </li>
            <li role="presentation">
                <a href="javascript:reloadWithParam('table','participations')">Научные мероприятия</a>
            </li>
        <?php } else { ?>
            <li role="presentation">
                <a href="javascript:reloadWithParam('table','publications')">Публикации</a>
    ...
    <div role="tabpanel">
        <ul class="nav nav-tabs" role="tablist">
            <li role='presentation' class='<?= $selectedAuthor ? 'active' : ''; ?>'>
                <a href="#my" aria-controls='my' role='tab'
                   data-toggle='tab' onclick="reloadWithParam('author', <?= $userid; ?>); return false;">Мои</a>
            </li>
            <li role='presentation' class="<?= !$selectedAuthor ? 'active' : ''; ?>">
                <a href="#all" aria-controls='all' role='tab' data-toggle='tab'
                   onclick="reloadWithParam('author',0); return false;">Все</a>
            </li>
            <li role='presentation'>
                <a href="#add" aria-controls='add' role='tab' data-toggle='tab'
                   onclick="showBlock('<?= $table; ?>'); return true;">Добавить</a>
            </li>
        </ul>
    </div>
    ...
    <div class="tab-content">
                ...
                foreach ($publications as $publication) {
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            WHERE authorpublications.publicationid = $publication->id AND authors.id <> $selectedAuthor
                        ...
                        <td>
                            <?= $publication->type == 1 ? "$publication->journalpagestart-$publication->journalpageend" :
                                $publication->numpages; ?>
                        </td>
                        <td>
                            <a href="javascript:del('publications','<?= $publication->id; ?>')">
        ...
        <?php } else { ?>
            <table class="table table-striped">
                ...
                $cntr = 1;
                if ($selectedAuthor)
                    $query = "SELECT participations.id, scevents.name, scevents.date, scevents.place, scevents.year
                    FROM participations
                ...
                foreach ($participations as $participation) {
                    ...
                    <tr>
                        ...
                        <td>
                            <a href="javascript:del('participations','<?= $participation->id; ?>');">
                                <span class="glyphicon glyphicon-remove"></span>
                            </a>
                        </td>
                    </tr>
